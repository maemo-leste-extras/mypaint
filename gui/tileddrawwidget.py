...
class TiledDrawWidget(gtk.DrawingArea):
    ...
    def state_changed_cb(self, widget, oldstate):
        ...
        sensitive = True  #get_state needs gtk 2.18
        if sensitive:
            self.snapshot_pixmap = None
        else:
    ...
    def motion_notify_cb(self, widget, event, button1_pressed=None):
        ...
        if pressure is None:
            ...
            if button1_pressed:
                ...
                pressure = 0.0
        #This causes ghost lines on Maemo.
        #else:
    ...
    def expose_cb(self, widget, event):
        ...
        if self.snapshot_pixmap:
            #gc = self.get_style().fg_gc[self.get_state()] #gtk_state needs gtk 2.18
            gc = self.get_style().fg_gc[0]  #0=GTK_STATE_NORMAL
            area = event.area
